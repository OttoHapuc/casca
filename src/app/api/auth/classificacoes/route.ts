...
export async function GET() {
  try {
    ...
    return NextResponse.json<RespostaPadrao>(
      { sucesso: false, mensagem: 'Erro ao buscar classificações' },
      { status: 500 },
    )
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    if (!nome)
      return NextResponse.json({ sucesso: false, mensagem: 'Nome é obrigatório' }, { status: 400 })
...
export async function DELETE(request: Request) {
  try {
    ...
    return NextResponse.json<RespostaPadrao>(
      { sucesso: false, mensagem: 'Erro ao excluir' },
      { status: 500 },
    )
  }
}
